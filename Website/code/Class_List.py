...
class Graph(object):
    ...
    def __init__(self, polygons):
        ...
        polygon_id = 0
        for input_polygon in polygons:
            #if len(input_polygon) == 1:
            #    self.graph[input_polygon[0]]
            if input_polygon[0] == input_polygon[-1] and len(input_polygon) > 1:
                input_polygon.pop()

            for i, vertex in enumerate(input_polygon):
                next_vertex = input_polygon[(i + 1) % len(input_polygon)]
                current_edge = Edge(vertex, next_vertex)

                if len(input_polygon) > 2:
                    vertex.polygon_id = polygon_id
                    next_vertex.polygon_id = polygon_id
                    self.polygons[polygon_id].add(current_edge)

                self.add_edge(current_edge)
